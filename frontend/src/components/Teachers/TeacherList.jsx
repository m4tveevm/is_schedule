...
//                 setNewTeachers([]);
//                 loadTeachers();
//                 toast.success('Новые преподаватели успешно добавлены!');
//             })
//             .catch((error) => {
...
//             <p>
//                 Здесь вы можете искать, сортировать, добавлять или удалять преподавателей. Также доступен импорт из
//                 Excel.
//             </p>
//
...
//                         <option value="surname">Сортировать по дате</option>
//                     </select>
//                 </div>
//             </div>
//
//             {filteredTeachers.length === 0 && newTeachers.length === 0 && (
//                 <div className="alert alert-info">Нет преподавателей. Добавьте или импортируйте.</div>
//             )}
//
...
//                                 </p>
//                                 <Link to={`/teachers/${t.id}/edit`} className="btn btn-sm btn-warning me-2">
//                                     Изменить
//                                 </Link>
//                             </div>
//                         </div>
...
function TeacherList() {
    ...
    const handleSaveNewTeachers = () => {
        const nonEmptyTeachers = newTeachers.filter(nt =>
        ...
        const validTeachers = nonEmptyTeachers.filter(nt =>
            nt.surname.trim() !== '' && nt.name.trim() !== '' && nt.lastname.trim() !== ''
        );
        const incompleteTeachers = nonEmptyTeachers.filter(nt =>
            (nt.surname.trim() === '' || nt.name.trim() === '' || nt.lastname.trim() === '') &&
            (nt.surname.trim() !== '' || nt.name.trim() !== '' || nt.lastname.trim() !== '')
        );
        ...
        Promise.all(validTeachers.map(nt => createTeacher(nt)))
            .then(() => {
                const remaining = newTeachers.filter(nt =>
                    !(nt.surname.trim() !== '' && nt.name.trim() !== '' && nt.lastname.trim() !== '')
                );
    ...
    return (
        <div className="container">
            ...
            <h1>Управление Сотрудниками</h1>
            <p>
                Здесь вы можете искать, сортировать, добавлять или удалять преподавателей. Также доступен импорт из Excel.
            </p>
                    ...
                    {newTeachers.map((nt, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <button className="btn btn-sm btn-outline-danger" onClick={() => handleRemoveNewTeacher(index)}>
                                    ✕
                                </button>
                                {!isEmpty && !isComplete && (
                                    <small className="text-warning">Заполните Фамилию, Имя и Отчество</small>
                                )}
                            </div>
                        );
                    })}
