...
function EducationalPlanList() {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            alert('Не удалось удалить план. Попробуйте позже.');
        }
    };
    ...
    return (<div className="container">
        ...
        <p className="text-muted">
            Здесь вы можете просмотреть, добавить или редактировать учебные
            планы. Используйте поиск для быстрого
            перехода к нужному плану.
        </p>
        <div className="actions mb-3 d-flex align-items-center">
            ...
            <div className="position-relative w-100">
                ...
                />
                {suggestions.length > 0 && (<SearchSuggestions
        ...
        {!loading && !error && plans.length === 0 && (
            <div className="alert alert-info">Учебные планы отсутствуют.
                Добавьте новый план.</div>)}
        <div className="row">
            {plans.map((plan) => (<div key={plan.id} className="col-md-4">
                <div className="card mb-3">
                    <div className="card-body">
                        <h5 className="card-title">{plan.name}</h5>
                        <p className="card-text">{plan.description || 'Без описания'}</p>
                        {plan.has_practice && (<p className="text-info">
                            Практика: {plan.practice_start_date} - {plan.practice_end_date}
                        </p>)}
                        <div className="d-flex justify-content-between">
                            <Link to={`/educational_plans/${plan.id}/edit`}
                                  className="btn btn-sm btn-warning">
                                Изменить
                            </Link>
                            <button className="btn btn-sm btn-danger"
                                    onClick={() => handleDelete(plan.id)}>Удалить
                            </button>
                        </div>
                    </div>
                </div>
