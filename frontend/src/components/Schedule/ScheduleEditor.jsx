        ...
        assignment.brigades.forEach((brigade) => {
            if (brigade.teacher_id) {
                if (usedTeachers.has(brigade.teacher_id)) {
                    errors[`brigade_${brigade.number}`] = `Преподаватель уже назначен для другой бригады`;
                } else {
                    ...
                    <div key={index} className="card mb-4">
                        ...
                        <div className="card-body">
                            ...
                            {assignment.brigades.map((brigade) => (
                                <div className="form-group"
                                     key={brigade.number}>
                                    <label>Преподаватель для
                                        бригады {brigade.number}</label>
                                    <select
                                        className={`form-control ${
                                            errorFields[index]?.[`brigade_${brigade.number}`] ? 'is-invalid' : ''
                                        }`}
                                    ...
                                    >
                                        ...
                                        {getAvailableTeachers(index, brigade.number).map((teacher) => (
