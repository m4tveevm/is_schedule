...
function TeacherUnavailableDatesList() {
    ...
    const loadTeachers = () => {
        setLoading(true);
        setError(null);
        axios.get(`${API_BASE_URL}teachers/?search=${encodeURIComponent(searchTerm)}`)
    ...
    return (
        <div className="container">
            ...
            <p className="text-muted">
                Здесь вы можете посмотреть преподавателей и отредактировать
                периоды, в которые они недоступны.
            </p>
            ...
            {!loading && !error && teachers.length === 0 && (
                <div className="alert alert-info">Преподаватели не
                    найдены.</div>
            )}
            <div className="row">
                {teachers.map(t => (
                    <div key={t.id} className="col-md-4">
                        <div className="card mb-3">
                            <div className="card-body">
                                ...
                                <p className="card-text">Тип: {t.employerType}</p>
                                <Link
