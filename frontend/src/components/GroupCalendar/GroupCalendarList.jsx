...
function GroupCalendarList() {
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((groupsData) => {
                const requests = groupsData.map((g) => axios
                    .get(`${API_BASE_URL}educational_plans/remaining?group_id=${g.id}`)
                    .then((resp) => ({
                        groupId: g.id, data: resp.data
                    }))
                    .catch(() => null));
    ...
    return (<div className="container">
        ...
        <p className="text-muted">
            Выберите группу, чтобы открыть календарь расписания и назначить
            занятия.
        </p>
        ...
        <div className="row">
            {groups.map((g) => {
                ...
                return (<div key={g.id} className="col-md-4">
                    <div className="card mb-3">
                        <div className="card-body">
                            <h5 className="card-title">{g.name}</h5>
                            <p className="card-text">
                                Остаток: УП {loadInfo['УП'] || 0} /
                                КЛ {loadInfo['КЛ'] || 0} /
                                ДК {loadInfo['ДК'] || 0}
                            </p>
                            <Link to={`/group_calendar/${g.id}`}
