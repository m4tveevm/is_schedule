...
class TimeSlot(models.Model):
    schedule_date = models.ForeignKey(
        ScheduleDate, on_delete=models.CASCADE, related_name="time_slots"
    )
...
class Lecture(models.Model):
    time_slot = models.ForeignKey(
        TimeSlot, on_delete=models.CASCADE, related_name="lectures"
    )
    ...
    def clean(self):
        if Lecture.objects.filter(
            ...
            raise ValueError(
                f"Преподаватель {self.teacher.name} уже назначен на "
                f"{self.time_slot.schedule_date.date}"
            )

    def __str__(self):
        group_names = ", ".join(group.name for group in self.groups.all())
        return (
            f"{self.teacher.name} - {self.time_slot} для групп "
            f"[{group_names}]"
        )
