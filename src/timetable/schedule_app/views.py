...
from .forms import DateSelectionForm, ScheduleDateFormSet, \
    LectureAssignmentForm
from django.forms import formset_factory
from .models import ScheduleDate, Lecture, TimeSlot, Group, Teacher
...
def date_selection_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['schedule_date_ids'] = [sd.id for sd in
                                                    schedule_dates]
...
def adjust_lectures_view(request):
    ...
    return render(request, 'schedule_app/adjust_lectures.html',
                  {'formset': formset})
...
def assign_teachers_view(request):
    ...
    if request.method == 'POST':
        ...
        # Передаём пары форм и временных слотов в шаблон
...
def schedule_view(request):
    lectures = Lecture.objects.all().order_by('time_slot__schedule_date__date',
...
def group_schedule_view(request, group_id):
    ...
    return render(request, 'schedule_app/group_schedule.html',
...
def teacher_schedule_view(request, teacher_id):
    ...
    lectures = Lecture.objects.filter(teacher=teacher).order_by(
        'time_slot__schedule_date__date', 'time_slot__slot_number')
    return render(request, 'schedule_app/teacher_schedule.html',
